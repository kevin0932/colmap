...
def main():
    ...
    with open(os.path.join(args.output_path), "w") as fid:
        ...
        for row in cursor:
            ...
            cursorDescriptors.execute("SELECT image_id, data FROM descriptors WHERE rows>=?;",
                           (args.min_num_matches,))
            for rowDes in cursorDescriptors:
                image_id = rowDes[0]
                print("rowDes[1] = ", np.fromstring(rowDes[1],dtype=np.uint8))
                print("image_id = ", image_id)
                descriptors = np.fromstring(rowDes[1],dtype=np.uint8).reshape(-1, 128)
